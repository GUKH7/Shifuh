-- Unify issued benefits at checkout while preserving source-specific audit trails.
-- Roulette keeps spin references. Loyalty keeps point/redemption records and links one generic checkout benefit.
...
begin
  ...
  perform pg_advisory_xact_lock(
    hashtextextended(v_reward.restaurant_id::text || ':' || v_idempotency_key, 0)
  );
  ...
  if v_reward.reward_validity_days is not null then
    v_expires_at := now() + make_interval(days => v_reward.reward_validity_days);
  end if;

  insert into public.loyalty_point_transactions as lpt (
    account_id,
    restaurant_id,
    program_id,
    customer_id,
  ...
  returning lpt.balance_after into v_balance_after;
...
comment on function public.redeem_loyalty_reward(uuid, text, text) is
  'Atomically validates a loyalty catalog item, debits points once and emits a generic checkout benefit.';
