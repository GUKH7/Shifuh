...
begin
  ...
  -- Use the same transaction-scoped lock as rewarded orders so ordinary and rewarded
  -- attempts sharing a restaurant/idempotency key cannot race one another.
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_restaurant_id::text || ':' || normalized_idempotency_key, 0)
  );
  ...
  if found then
    if exists (
      ...
      where cr.restaurant_id = p_restaurant_id
        and cr.redeemed_order_id = created_order.order_id
    ) then
      raise exception using errcode = '23514', message = 'Idempotency key already used with a reward';
    end if;
