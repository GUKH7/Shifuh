-- Review hardening for loyalty redemption discovery and checkout provenance.
-- Keeps historical customer phone formats discoverable and preserves source-aware free-product labels.

create or replace function public.find_loyalty_customers_by_phone(
...
begin
  ...
  if v_reward.reward_type = 'percent' then
    ...
    v_reward_observation := case
      when v_reward.source_type = 'loyalty' then 'Recompensa do Programa de Fidelidade'
      else 'Prêmio da Roleta da Sorte'
    end;
    ...
    v_items := v_items || pg_catalog.jsonb_build_array(pg_catalog.jsonb_build_object(
      ...
      'quantity', 1,
      'price', 0,
      'observation', v_reward_observation,
      'addons', '[]'::jsonb
    ));
  else
...
comment on function public.find_loyalty_customers_by_phone(text, uuid) is
  'Service-role helper that resolves historical customer phone formatting without broad client-visible customer scans.';
