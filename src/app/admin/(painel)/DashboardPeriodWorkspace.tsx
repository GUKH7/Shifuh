  ...
  return (
    <AdminPageShell className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {Array.from({ length: 5 }).map((_, index) => <AdminSkeleton key={index} className="h-32 w-full" />)}
      </div>
    ...
    <AdminPageShell className="space-y-5 sm:space-y-6">
      <AdminPageHeader
        ...
        action={
          <div className={`flex w-full items-center gap-3 rounded-[22px] border px-4 py-3 shadow-sm sm:w-auto ${storeTone.shell}`}>
            <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${storeTone.icon}`}><Store size={20} /></div>
            <div className="min-w-0 flex-1 sm:min-w-[150px]">
              <p className={`font-black ${storeTone.title}`}>{storeTone.label}</p>
              <div className="mt-1 flex items-center gap-2 text-xs text-gray-500"><span>{storeTone.helper}</span><LiveStatusDot className={storeTone.dot} /></div>
            </div>
            <Link href={restaurantSlug ? `/${restaurantSlug}` : "/admin/settings"} target={restaurantSlug ? "_blank" : undefined} className="hidden min-h-10 items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-3 text-xs font-bold text-gray-700 transition-colors hover:text-gray-950 sm:inline-flex">Ver loja <ExternalLink size={14} /></Link>
            <Link href="/admin/settings" aria-label="Abrir configurações da loja" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[var(--line)] bg-white text-gray-500 transition-colors hover:text-gray-950"><MoreVertical size={17} /></Link>
          </div>
        }
      />
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <AdminSelect id="dashboard-period" value={period} onChange={(event) => setPeriod(event.target.value as DashboardPeriod)} className="min-h-11 w-[180px] py-2 text-sm" aria-label="Período global do dashboard">
            {DASHBOARD_PERIODS.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
          </AdminSelect>
        </div>
      </div>
      ...
      <section className="grid gap-4 xl:grid-cols-12">
        <article className="surface-card rounded-[24px] p-4 sm:p-5 xl:col-span-5">
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-base font-black text-gray-950">Faturamento</p><p className="mt-1 text-xs text-gray-400">Evolução das vendas no período</p></div>
            <div className="text-right"><p className="text-lg font-black text-gray-950">{formatMoney(dashboard.periodRevenue)}</p><p className="text-xs font-semibold text-emerald-600">{dashboard.range.label}</p></div>
          </div>
          <div className="mt-5 h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={dashboard.revenueSeries} margin={{ top: 12, right: 8, left: -16, bottom: 0 }}>
                <defs><linearGradient id="dashboardRevenuePeriod" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ff6a00" stopOpacity={0.28} /><stop offset="100%" stopColor="#ff6a00" stopOpacity={0.02} /></linearGradient></defs>
        ...
        <article className="surface-card rounded-[24px] p-4 sm:p-5 xl:col-span-3">
          <div className="flex items-start justify-between gap-3"><div><p className="text-base font-black text-gray-950">Produtos mais pedidos</p><p className="mt-1 text-xs text-gray-400">Ranking por quantidade</p></div><span className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-xs font-bold text-gray-500">{dashboard.range.label}</span></div>
          {dashboard.topProducts.length === 0 ? <div className="mt-5 flex h-[250px] items-center justify-center rounded-2xl border border-dashed border-[var(--line)] bg-white px-5 text-center text-sm text-gray-500">Nenhum produto vendido no período.</div> : (
            <div className="mt-5 h-[250px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={dashboard.topProducts} layout="vertical" margin={{ top: 4, right: 18, left: 4, bottom: 4 }}><XAxis type="number" hide /><YAxis type="category" dataKey="name" axisLine={false} tickLine={false} width={118} tick={{ fontSize: 10, fill: "#554c46" }} /><Tooltip formatter={(value) => `${Number(value)} unidade(s)`} /><Bar dataKey="quantity" name="Quantidade" fill="#ff6a00" radius={[0, 8, 8, 0]} barSize={11} /></BarChart></ResponsiveContainer></div>
          )}
        </article>

        <article className="surface-card rounded-[24px] p-4 sm:p-5 xl:col-span-4">
          <p className="text-base font-black text-gray-950">Fontes de pedidos</p><p className="mt-1 text-xs text-gray-400">Origem dos pedidos válidos em {dashboard.range.label.toLowerCase()}</p>
          {dashboard.sources.length === 0 ? <div className="mt-5 flex h-[250px] items-center justify-center rounded-2xl border border-dashed border-[var(--line)] bg-white px-5 text-center text-sm text-gray-500">Nenhum pedido registrado no período.</div> : (
            <div className="mt-3 grid items-center gap-3 sm:grid-cols-[1fr_150px]">
              <div className="relative h-[250px] min-w-0"><ResponsiveContainer width="100%" height="100%"><PieChart><Tooltip formatter={(value) => `${Number(value)} pedido(s)`} /><Pie data={dashboard.sources} dataKey="count" nameKey="label" cx="50%" cy="50%" innerRadius={56} outerRadius={84} paddingAngle={1}>{dashboard.sources.map((source) => <Cell key={source.key} fill={source.color} />)}</Pie></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center"><p className="text-3xl font-black text-gray-950">{dashboard.sourceTotal}</p><p className="text-xs text-gray-400">pedidos</p></div></div>
              <div className="space-y-3">{dashboard.sources.map((source) => { const percentage = dashboard.sourceTotal > 0 ? (source.count / dashboard.sourceTotal) * 100 : 0; return <div key={source.key} className="flex items-center gap-2 text-xs"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: source.color }} /><span className="min-w-0 flex-1 truncate font-semibold text-gray-600">{source.label}</span><span className="shrink-0 font-bold text-gray-950">{source.count} ({percentage.toFixed(0)}%)</span></div>; })}</div>
            </div>
          )}
        </article>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-12">
        <article className="surface-card rounded-[24px] p-4 sm:p-5 xl:col-span-5">
          <div className="flex items-start justify-between gap-4"><div><p className="text-base font-black text-gray-950">Atividade por horário</p><p className="mt-1 text-xs text-gray-400">Pedidos distribuídos por hora em {dashboard.range.label.toLowerCase()}</p></div><span className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-xs font-bold text-gray-500">{dashboard.range.label}</span></div>
          <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_155px]">
            <div className="h-[245px] min-w-0"><ResponsiveContainer width="100%" height="100%"><BarChart data={dashboard.hourlyActivity} margin={{ top: 12, right: 4, left: -26, bottom: 0 }}><CartesianGrid strokeDasharray="4 4" stroke="#efe7df" vertical={false} /><XAxis dataKey="label" axisLine={false} tickLine={false} interval={3} tick={{ fontSize: 10, fill: "#8c837c" }} /><YAxis axisLine={false} tickLine={false} allowDecimals={false} tick={{ fontSize: 10, fill: "#8c837c" }} /><Tooltip formatter={(value) => `${Number(value)} pedido(s)`} /><Bar dataKey="orders" name="Pedidos" fill="#ff6a00" radius={[6, 6, 0, 0]} barSize={8} /></BarChart></ResponsiveContainer></div>
            <div className="grid content-center gap-3 sm:grid-cols-3 lg:grid-cols-1">
              <div className="rounded-2xl bg-[#fff7f1] p-4"><Clock3 size={17} className="text-[var(--brand)]" /><p className="mt-3 text-xs font-bold text-gray-400">Pico no período</p><p className="mt-1 text-lg font-black text-gray-950">{dashboard.peakHour.label} – {String((dashboard.peakHour.hour + 1) % 24).padStart(2, "0")}h</p><p className="mt-1 text-xs text-gray-500">{dashboard.peakHour.orders} pedido(s)</p></div>
              <div className="rounded-2xl bg-[#fff7f1] p-4"><PackageCheck size={17} className="text-[var(--brand)]" /><p className="mt-3 text-xs font-bold text-gray-400">Em preparo agora</p><p className="mt-1 text-lg font-black text-gray-950">{dashboard.inKitchen}</p></div>
              <div className="rounded-2xl bg-[#fff7f1] p-4"><Bike size={17} className="text-[var(--brand)]" /><p className="mt-3 text-xs font-bold text-gray-400">Em rota agora</p><p className="mt-1 text-lg font-black text-gray-950">{dashboard.onTheWay}</p></div>
            </div>
          </div>
        </article>

        <article className="surface-card overflow-hidden rounded-[24px] xl:col-span-7">
          <div className="flex items-center justify-between gap-4 border-b border-[var(--line)] px-4 py-4 sm:px-5"><div><p className="text-base font-black text-gray-950">Pedidos recentes</p><p className="mt-1 text-xs text-gray-400">Últimos pedidos de {dashboard.range.label.toLowerCase()}</p></div><Link href="/admin/orders" className="text-xs font-black text-[var(--brand)] hover:underline">Ver todos</Link></div>
          {dashboard.recentOrders.length === 0 ? <div className="flex min-h-[270px] items-center justify-center px-6 text-center text-sm text-gray-500">Nenhum pedido registrado no período.</div> : (
            <>
              <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[680px] border-collapse text-sm"><thead><tr className="border-b border-[var(--line)] text-left text-[11px] font-black uppercase tracking-[0.12em] text-gray-400"><th className="px-5 py-3">Pedido</th><th className="px-3 py-3">Cliente</th><th className="px-3 py-3">Origem</th><th className="px-3 py-3">Horário</th><th className="px-3 py-3">Valor</th><th className="px-3 py-3">Status</th><th className="w-10 px-3 py-3" /></tr></thead><tbody className="divide-y divide-[var(--line)]">{dashboard.recentOrders.map((order) => { const source = SOURCE_META[resolveSource(order)]; const status = STATUS_META[order.status]; return <tr key={order.id} className="text-gray-600 transition-colors hover:bg-[#fffaf6]"><td className="px-5 py-3.5 font-black text-gray-950">#{order.display_number || order.id.slice(0, 5)}</td><td className="max-w-[150px] truncate px-3 py-3.5 font-semibold text-gray-700">{order.customer_name}</td><td className="px-3 py-3.5"><span className="inline-flex items-center gap-2 whitespace-nowrap"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: source.color }} />{source.label}</span></td><td className="whitespace-nowrap px-3 py-3.5">{formatDateTime(order.created_at)}</td><td className="whitespace-nowrap px-3 py-3.5 font-bold text-gray-950">{formatMoney(order.total)}</td><td className="px-3 py-3.5"><span className={`inline-flex whitespace-nowrap rounded-full border px-3 py-1 text-xs font-bold ${status.className}`}>{status.label}</span></td><td className="px-3 py-3.5 text-gray-400"><MoreVertical size={16} /></td></tr>; })}</tbody></table></div>
              <div className="divide-y divide-[var(--line)] md:hidden">{dashboard.recentOrders.map((order) => { const source = SOURCE_META[resolveSource(order)]; const status = STATUS_META[order.status]; return <div key={order.id} className="space-y-3 px-4 py-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-black text-gray-950">#{order.display_number || order.id.slice(0, 5)}</p><p className="mt-1 truncate text-sm font-semibold text-gray-600">{order.customer_name}</p></div><p className="shrink-0 font-black text-gray-950">{formatMoney(order.total)}</p></div><div className="flex flex-wrap items-center gap-2 text-xs text-gray-500"><span className="inline-flex items-center gap-2 rounded-full bg-[#f8f3ec] px-3 py-1.5"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: source.color }} />{source.label}</span><span className="inline-flex items-center gap-1 rounded-full bg-[#f8f3ec] px-3 py-1.5"><Clock3 size={13} /> {formatHour(order.created_at)}</span><span className={`inline-flex rounded-full border px-3 py-1.5 font-bold ${status.className}`}>{status.label}</span></div></div>; })}</div>
            </>
          )}
        </article>
      </section>
