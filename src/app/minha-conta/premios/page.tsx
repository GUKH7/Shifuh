...
import { ArrowLeft, ArrowRight, CalendarClock, CheckCircle2, Gift, Loader2, Package, Sparkles, TicketPercent, Truck, WalletCards } from "lucide-react";
      ...
      <div className="mx-auto max-w-3xl px-4 pt-6 sm:pt-8">
        <section className="overflow-hidden rounded-3xl bg-gray-950 p-6 text-white shadow-[0_18px_55px_rgba(17,24,39,0.16)] sm:p-8">
          <div className="flex items-start justify-between gap-5">
            <div>
              <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10 text-orange-400"><Gift size={25} /></span>
              <h2 className="mt-5 text-2xl font-black sm:text-3xl">Suas recompensas em um só lugar</h2>
              <p className="mt-2 max-w-xl text-sm leading-6 text-white/65">Benefícios resgatados na Fidelidade e prêmios conquistados na Roleta ficam aqui até serem utilizados ou expirarem.</p>
            </div>
            <div className="shrink-0 rounded-2xl bg-white/10 px-4 py-3 text-center"><strong className="block text-2xl font-black text-orange-400">{availableCount}</strong><span className="text-[10px] font-bold uppercase tracking-wider text-white/55">disponíveis</span></div>
          </div>
        </section>

        {loading && (
          <div className="mt-6 flex min-h-48 items-center justify-center rounded-2xl border border-gray-200 bg-white">
            <div className="text-center text-gray-500"><Loader2 className="mx-auto animate-spin text-orange-500" size={26} /><p className="mt-3 text-sm font-bold">Carregando seus prêmios...</p></div>
          </div>
        )}

        {!loading && error && <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm font-semibold text-rose-700">{error}</div>}
        ...
        {!loading && !error && sortedRewards.length > 0 && (
          <section className="mt-7">
            <div className="mb-3 flex items-center justify-between px-1">
              <div><p className="text-xs font-black uppercase tracking-[0.14em] text-gray-400">Carteira</p><h2 className="mt-1 text-xl font-black">Seus prêmios</h2></div>
            ...
            <div className="space-y-3">
              {sortedRewards.map((reward) => {
                ...
                const fromLoyalty = reward.source === "loyalty";
                return (
                  <article key={reward.id} className={`rounded-2xl border bg-white p-5 shadow-[0_7px_24px_rgba(17,24,39,0.04)] ${available ? "border-orange-100" : "border-gray-200 opacity-75"}`}>
                    <div className="flex items-start gap-4">
                      <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${available ? "bg-orange-50 text-orange-600" : "bg-gray-100 text-gray-500"}`}><RewardIcon type={reward.type} /></span>
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="truncate text-xs font-bold text-gray-400">{reward.restaurant?.name || "Loja"}</p>
                            <div className="mt-1 flex flex-wrap items-center gap-2">
                              <h3 className="text-lg font-black leading-tight text-gray-950">{reward.label}</h3>
                              <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider ${fromLoyalty ? "bg-orange-50 text-orange-700" : "bg-violet-50 text-violet-700"}`}>{fromLoyalty ? "Fidelidade" : "Roleta"}</span>
                            </div>
                          </div>
                        ...
                        {fromLoyalty && Number(reward.pointsSpent || 0) > 0 && <p className="mt-1 text-xs font-semibold text-gray-400">Resgatado por {reward.pointsSpent} pontos</p>}

                        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-gray-500">
                          {expires && <span className="inline-flex items-center gap-1.5"><CalendarClock size={14} /> {available ? `Válido até ${expires}` : `Validade ${expires}`}</span>}
                          {reward.status === "redeemed" && reward.redeemedAt && <span className="inline-flex items-center gap-1.5"><CheckCircle2 size={14} /> Usado em {formatDate(reward.redeemedAt)}</span>}
                        </div>

                        {available && reward.restaurant?.slug && (
                          <button type="button" onClick={() => router.push(`/${reward.restaurant!.slug}`)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl border border-gray-200 px-4 py-3 text-sm font-black text-gray-800 transition hover:border-orange-200 hover:bg-orange-50 hover:text-orange-700">Ir para a loja <ArrowRight size={17} /></button>
                        )}
                      </div>
                    </div>
                  </article>
